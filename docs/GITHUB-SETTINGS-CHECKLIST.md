...
- [ ] **Require status checks to pass** before merging, including at least:
  ...
  - [ ] Optional: **CodeQL** / **OpenSSF Scorecard** jobs if those workflows are enabled
...
- [ ] **Dependabot security updates** enabled; use **grouped security updates** (see `.github/dependabot.yml` for grouped config).
- [ ] **Code scanning** — enable **CodeQL** for PHP (workflow `.github/workflows/codeql.yml` ships analysis; turn on “Code scanning” in the Security tab if prompted).

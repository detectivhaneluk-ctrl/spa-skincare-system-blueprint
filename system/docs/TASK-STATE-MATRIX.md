...
- BIG-05 (2026-03-31 / **APPOINTMENTS REGRESSION KILL-SHOT**): Post-BIG-04 calendar AJAX regression fixed. Root cause: `BlockedSlotRepository::listGroupedByStaffForDate()` and `listForStaffAndDate()` used `FROM appointment_blocked_slots` without the `bs` alias, while `appendBlockedSlotBranchTenantClause()` generated `bs.branch_id` fragments — MySQL "Unknown column" `PDOException` on every `dayCalendar()` AJAX request → HTTP 500 "An error occurred." Fix: both FROM clauses now use `FROM appointment_blocked_slots bs` with aliased column references. `listForDate()` (called by `dayCalendarPage()`) already had the alias and was unaffected. Guardrail check added: `BlockedSlotRepository all FROM clauses carry bs alias` in `verify_root_01_id_only_closure_wave_plt_tnt_01.php`. Branch selector source (`BranchDirectory::getActiveBranchesForSelection()`) confirmed canonical and org-scoped — no code change needed. **Known BIG-04 residual surfaced:** `AppointmentService` mutation paths still call `repo->find($id)` rather than `repo->findForUpdate($id)`; `AppointmentRepository::findForUpdate()` exists but the service-level migration is incomplete. Documented as `[BIG-04 residual]` in the verification script; out of scope for BIG-05.

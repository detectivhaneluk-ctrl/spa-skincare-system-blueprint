...
chk('A10c: Reports path uses navIsReports for Overview active state', $navB, '$navIsReports');
chk('A10d: primary nav has no Catalog module entry', $navB, "'/services-resources', 'Catalog'", false);
chk('A10e: no primary nav Reports home tuple', $navB, "'/reports', 'Reports'", false);
chk('A10f: Admin active includes catalog definition surfaces', $navB, '$navIsSettings = $navIsSettings || $navIsCatalog');
chk('A10g: Overview highlights /reports via navIsReports', $navB, "str_starts_with(\$navPath, '/dashboard') || \$navIsReports");

...
/**
 * Purge expired trashed services for one organization or all organizations (cron-safe).
 *
 * Eligibility: deleted_at IS NOT NULL AND purge_after_at <= now (UTC app timezone from config).
 * Skips rows still referenced by appointment_series; logs structured warnings via slog().
 *
 * From repo root:
 *   php system/scripts/purge_services_trash_cli_01.php --organization-id=1
 *   php system/scripts/purge_services_trash_cli_01.php --all-organizations --batch=50
 *   php system/scripts/purge_services_trash_cli_01.php --organization-id=1 --repeat-until-empty
 */
...
foreach ($orgRows as $or) {
    ...
    if ($br === null) {
        fwrite(STDERR, "purge_services_trash: skip org {$oid} (no live branch)\n");
        continue;
    }
    $bid = (int) $br['id'];
    ...
    $iter = 0;
    do {
        $r = $service->purgeExpiredTrashedBatch($batch);
        ...
        $iter++;
        if ($iter > 10000) {
            fwrite(STDERR, "purge_services_trash: safety stop after 10000 iterations (org {$oid})\n");
            break;
        }
...
fwrite(
    STDOUT,
    sprintf("purge_services_trash: total purged=%d skipped_blocked=%d skipped_error=%d\n", $grandPurged, $grandSkipB, $grandSkipE)
);
exit(0);

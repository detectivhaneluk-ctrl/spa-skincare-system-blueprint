...
<?php endif; ?>
<p><strong>Balance:</strong> <?= number_format((float)$currentBalance, 2) ?> <?= htmlspecialchars($giftCard['currency'] ?? '') ?></p>
...
<form method="post" action="/gift-cards/<?= (int)$giftCard['id'] ?>/redeem" class="entity-form">
    ...
    <div class="form-row"><label for="amount">Amount to redeem *</label><input type="number" min="0.01" step="0.01" id="amount" name="amount" required value="<?= htmlspecialchars((string)($redeem['amount'] ?? '')) ?>"></div>
    ...
    <div class="form-actions"><button type="submit">Redeem gift card</button> <a href="/gift-cards/<?= (int)$giftCard['id'] ?>">Cancel</a></div>
</form>

...
<section class="settings-establishment">
    <header class="settings-establishment__hero">
        <h2 class="settings-establishment__title">Edit VAT type</h2>
    ...
    <div class="settings-establishment-card settings-establishment-card--full">
        <p>Code: <code><?= htmlspecialchars((string) ($rate['code'] ?? '')) ?></code> (immutable)</p>
        <?php if (!empty($errors)): ?>
        ...
        <?php endif; ?>
        <form method="post" action="/settings/vat-rates/<?= (int) ($rate['id'] ?? 0) ?>" class="settings-form entity-form">
            ...
            <div class="settings-establishment-actions">
                <button type="submit" class="settings-establishment-btn settings-establishment-btn--primary">Update</button>

...
ob_start();
?>
<h1>Create compensation rule</h1>
<?php if (!empty($errors['_general'])): ?>
<div class="flash flash-error"><?= htmlspecialchars((string) $errors['_general']) ?></div>
<?php endif; ?>

<form method="post" action="/payroll/rules" class="stack-form">
    ...
    <label>Currency (fixed rules, must match invoice currency)
        <input type="text" name="currency" value="<?= htmlspecialchars((string) ($rule['currency'] ?? '')) ?>" placeholder="USD">
    </label>
    ...
    <label>Priority (higher wins ties)
        <input type="number" name="priority" value="<?= (int) ($rule['priority'] ?? 0) ?>">
    </label>
    <label><input type="checkbox" name="is_active" value="1" <?= ((int) ($rule['is_active'] ?? 1) === 1) ? 'checked' : '' ?>> Active</label>

    <p><button type="submit" class="btn">Save</button> <a href="/payroll/rules">Cancel</a></p>
</form>

<?php
// Standalone edit page — the primary edit flow now goes through the index panel (?edit=ID).
// This page handles direct URL access and is still used for old links.
$title = 'Edit Service Category: ' . htmlspecialchars($category['name'] ?? '');
ob_start();
$selectedParentId = isset($category['parent_id']) && $category['parent_id'] !== '' && $category['parent_id'] !== null
...
<div class="taxmgr-standalone-wrap">
    <div class="taxmgr-standalone-header">
        ...
        <h1>Edit: <?= htmlspecialchars($category['name'] ?? '') ?></h1>
    </div>
    <?php if (!empty($errors)): ?>
    ...
    <?php endif; ?>
    <form method="post" action="/services-resources/categories/<?= (int) $category['id'] ?>" class="taxmgr-form taxmgr-form--standalone">
        ...
        <div class="taxmgr-form-row">
            ...
            <span class="taxmgr-field-hint">Self and descendants are excluded (cycle prevention).</span>
        </div>
        ...
        <div class="taxmgr-form-actions">
            <button type="submit" class="btn taxmgr-btn-primary">Save Changes</button>
            <a href="/services-resources/categories/<?= (int) $category['id'] ?>" class="btn btn-ghost">View</a>
